...
{
    if (camIdx >= m_camOdoThreads.size()) return;   // 如果相机ID大于相机线程数，则退出；
    if (!m_camOdoThreads[camIdx] || m_camOdoThreads[camIdx]->running()) return;     // 如果当前相机线程无效或正在运行，则退出；

    m_camOdoThreads[camIdx]->setCamOdoTransformEstimate(odoT);  // 设置当前相机线程初始姿态
}
...
{
    if (m_options.beginStage == 0)  // 从头开始标定
    {
        boost::asio::io_service io;     // boost io
        boost::asio::deadline_timer timer(io, boost::posix_time::milliseconds(100));    // boost deadline time
        bool closeWindow = false;   // 关闭窗口
